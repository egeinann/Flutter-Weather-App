import 'dart:ui';

class WeatherColors {
  static const Color sunny = Color(0xFFFFC93C);
  static const Color rainy = Color(0xFF22577E);
  static const Color cloudy = Color(0xFFB6BBC4);
  static const Color snowy = Color(0xFFFFF2F2);
  static const Color stormy = Color(0xFF27667B);
  static const Color windy = Color(0xFF9ACBD0);
  static const Color sandstorm = Color(0xFF948979);
}

class AppColors {
  static const Color background = Color(0xFFDBDFEA);
  static const Color button = Color(0xFF526D82);
  static const Color container = Color(0xFF6151c3);
  static const Color shadow = Color(0xFF212A3E);
  static const Color textDark = Color(0xFF222831);
  static const Color textLight = Color(0xFFF2F9FF);
  static const Color icon = Color(0xFF27374D);
  static const Color bigTextDark = Color(0xFF222831);
  static const Color bigTextLight = Color(0xFFF2F9FF);
}
